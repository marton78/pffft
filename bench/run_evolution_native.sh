#!/usr/bin/env bash
# Sequential direct-measurement sweep of the perf branch's evolution on
# local (Mac) + ios (connected iPhone). One collect_series.py invocation
# per commit, both targets, --serial-measure to avoid host/device
# contention. 30 sustained reps per (target,prec,size), first 10 discarded
# as warm-up (see bench/PERF_TESTING.md caveat #3).
set -euo pipefail
cd "$(dirname "$0")/.."
...
# label:ref pairs, oldest (master) to tip of perf branch. The WASM commit
# (e836207) is excluded here -- it is native no-op (see analysis) and is
# benched separately with emcc in run_evolution_wasm.sh.
STEPS=(
  "00-master:master"
